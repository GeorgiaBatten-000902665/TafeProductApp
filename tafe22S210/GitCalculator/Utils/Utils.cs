...
namespace Calculator.Utils
{
	...
	{
		// add comment here
		private static double executeExpression(string text) //change int to double
		{
			string number = new DataTable().Compute(text, "").ToString();
			double numberDouble = Convert.ToDouble(number);

			return numberDouble; //cahnge return to numberDouble;(return Convert.ToInt16(numberDouble)
		}
		...
		// if primary display and secondarydisplay under 50
		//check if secondary display isnumeric and primary display =0
		// else secondary display and first display and operations
		//else uf primary display = 0
		//secondary display and first display and operations
		// reset primary display text
